...
def test_home_endpoint():
    ...
    response = client.get("/api/advad-ai/")
...
def test_askai_without_token():
    ...
    response = client.post("/api/advad-ai/askai", json={"prompt": "Hola"})
...
def test_askai_wrong_token():
    ...
    response = client.post("/api/advad-ai/askai", headers=headers, json={"prompt": "Hola"})
    assert response.status_code == 403
    assert response.json() == {"error": "Access denied"}
...
def test_askai_empty_prompt():
    ...
    response = client.post("/api/advad-ai/askai", headers=headers, json={"prompt": ""})
    assert response.status_code == 400
    assert response.json() == {"error": "Prompt is required"}

@patch("app.routers.ai.genai.Client")
def test_askai_success(mock_generate_content):
    ...
    mock_generate_content.return_value.models.generate_content.return_value = MockGeminiResponse("¡Entendido, soldado! Mantenga la posición.")

    headers = {"X-App-Token": "test_super_secret_token"}
    payload = {"prompt": "Reporte de situación"}


    response = client.post("/api/advad-ai/askai", headers=headers, json=payload)
